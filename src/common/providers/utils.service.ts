...
export class UtilService {
  public async makeRequestWithRetries(
    url: string,
    maxAttemts: number = 3,
    attemptDelay: number = 5000,
  ): Promise<string> {
    let attempts = 0;
  ...
  public getDataFromJsonString<T>(
    jsonString: string,
    field: string,
    defaultValue: T,
  ): T | null {
    ...
    if (defaultValue instanceof Array)
      return this.parseArrayFromString(result) as unknown as T;
