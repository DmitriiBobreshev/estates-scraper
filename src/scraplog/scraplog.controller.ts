...
export class ScraplogController {
  constructor(private scrapService: ScrapStatusService) {}
  ...
  GetJobs(jobRequest: GetJobsRequest): Observable<GetJobsResponse> {
    ...
    this.scrapService.getStatuses(dateFromNum, dateToNum).then((res) => {
      res.forEach((s) => {
        job$.next({
          id: s._id.toString(),
          status: s.status,
  ...
  GetAllRecords(): Observable<LogRecord> {
    ...
    this.scrapService.getAllRecords().then((res) => {
      res.forEach((r) => {
        job$.next({
          recordId: r.id,
          jobId: r.scraperId,
  ...
  GetRecordByJobId(data: RecordIdRequest): Observable<LogRecord> {
    const job$ = new Subject<LogRecord>();
    this.scrapService.getRecordByJobId(data.JobId).then((res) => {
      res.forEach((r) => {
        job$.next({
          recordId: r.id,
          jobId: r.scraperId,
