...
export class HalooglasiService {
  ...
  private readonly logger = new Logger(HalooglasiService.name, { timestamp: true });
  ...
  private async scrapSingle(baseUrl: string, propType: PropertyType, listeningType: ListeningType) {
    ...
    while (true) {
      ...
      try {
        ...
        this.logger.verbose(`Scrapped ${specifiedEstates.length} records for ${propType} ${listeningType}`);
      } catch (e) {
        this.isErrorHappened = true;
        this.logger.error(`Failed to scrap page ${url} for ${propType} ${listeningType}, Error: ${e}`);
